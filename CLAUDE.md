...
scripts/deploy_box.py — deterministic Mac→box deploy (REQ-DEP-001..004): clean-worktree guard, gitignore-driven excludes, protect-filters on data/ reports/ .venv/; DRY-RUN default, `--apply --restart <units>`.
...
| `accounting-freshness-sentinel.timer` | timer | daily 13:45 UTC → data-level freshness/invariant assertions → n8n severity webhook (REQ-SEN); exit 1 only on sentinel infra failure, so OnFailure covers the watchdog itself. Writes a `freshness_sentinel` heartbeat row to ingestion_log per --apply run. |
