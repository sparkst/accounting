...
| **crm/workers-brokerage** | `feat/wealth-workers-brokerage` | Workers handlers under `src/routes/(wealth)/wealth/api/brokerage/*` (all 13 routes incl. top-holdings/recent-transactions/data-integrity, **plus REQ-WC-013a `GET /quotes`**); reconciliation summary; internal-ingest endpoints (`/wealth/api/internal/ingest/*`) with `X-Internal-Key` auth; **exports** `handleTwelveDataIngest(env, ctx)` and `handleR2Backup(env, ctx)` from `src/lib/server/wealth/`. The workers-brokerage PR also includes the matching edits to `worker.ts` that replace the d1-schema-authored stubs for these two handlers. Depends on crm/d1-schema. |
| **crm/frontend-brokerage** | `feat/wealth-frontend-brokerage` | `src/routes/(wealth)/wealth/+page.svelte` (Wealth dashboard); `(wealth)/wealth/networth`, `holdings`, `accounts`, `accounts/[id]`, `missing-accounts`; **REQ-WC-013a live-quote integration** (call `/wealth/api/brokerage/quotes?symbols=...` on mount + visibilitychange for visible-symbol prices; "Prices as of HH:MM" display from min(fetched_at); stale warning indicator when `is_stale: true` or `max(fetched_at) > 30 min`); ESLint `no-restricted-paths` rule isolating `(crm)` and `(wealth)`; Svelte 5 runes enforced via custom ESLint rule (`no-svelte4-syntax`). Depends on crm/workers-brokerage. |
...
**M0h — WAF rate-limit + Plaid redirect URI registration.** **Reconciliation (M0h sub-team 2026-05-11, 4/4 consensus):** the original M0h drafted Cloudflare Access dashboard work, but live inspection showed CF Access has zero applications/policies for this account and the CRM authenticates entirely in-app via Google OAuth + HMAC session cookie. CF Access dashboard work is dropped from M0h (and the Service Token / AUD-tag / JWKS-cache infrastructure dependent on it). The auth boundary spec is updated in REQ-WC-002 + A4 to ratify the in-app pattern. CF Access deployment moves to **TF-002** (existing follow-up).
...
Validator SUBSTANTIATES from the two committed screenshots.

**M0h does NOT (reconciled):**
- Create CF Access applications / policies / IdP / groups.
- Create a CF Access Service Token (the spec's `CF_SMOKE_TOKEN_ID`/`CF_SMOKE_TOKEN_SECRET` mechanism is also retired; step 7g/7k smoke-tests now use a valid session cookie generated against the deployed Pages site — exact mechanism documented at step 7g-pre).
- Configure `*.pages.dev` Access policy (the in-app `hooks.server.ts` cookie guard covers preview URLs by virtue of being in the same handler).
- Capture a `CF_ACCESS_AUD_TAG` (no CF Access app exists; nothing to capture).
...
| BR-T08 | (REQ-WC-013a) Live-quote endpoint `GET /wealth/api/brokerage/quotes?symbols=A,B,C`. Implementation:<br>• Parse comma-separated symbols, trim+uppercase, dedup. Max 50 per request → 413.<br>• Validate each symbol against `^[A-Z0-9.^]{1,12}$` regex → 400 with sanitized message on failure (do NOT echo the bad symbol back; just say "invalid symbol format").<br>• For each symbol: read `live_quote.fetched_at`. If within 15 minutes → return cached `{symbol, price, currency, fetched_at, source, is_stale}`.<br>• Else, determine market-open state via `Intl.DateTimeFormat('en-US', {timeZone:'America/New_York'})`: Mon-Fri 09:30-16:00 ET = open.<br>• Market closed → read latest `historical_price.close` for the symbol (most recent `trade_date`); return `{symbol, price, currency:'USD', fetched_at: now, source:'historical_price', is_stale: false}` (EOD is canonical when closed).<br>• Market open + budget available (check shared `source='twelve_data'` IngestionLog daily counter < 600) → fetch Twelve Data `quote` endpoint per symbol, upsert `live_quote` with `is_stale: 0`, return.<br>• Market open + budget exhausted → return previous `live_quote` (or `historical_price.close` fallback) with `is_stale: 1`; HTTP 200 (NOT 429 — degraded-mode contract per REQ-WC-013a).<br>• Write ONE IngestionLog row per HTTP request with `source='twelve_data'`, `records_processed=symbols_fetched`, `records_failed=symbols_failed`, `error_detail` as JSON blob of failed symbols if any.<br>• Apply the same URL-sanitization helper from BR-T04 to every fetch call site.<br>• Defense in depth: the route handler re-checks `event.locals.user.email` against `WEALTH_ALLOWED_EMAILS` (do not rely solely on `hooks.server.ts`). | Tests (see REQ-WC-013a KAT list, all 8 cases): cache hit (zero API calls); cache miss + market open + budget available (N API calls); 51 symbols → 413; injection symbol → 400; market closed → zero API calls, returns historical_price.close; budget exhausted → zero API calls, returns stale; unauthenticated → 302; non-allowlisted email → 404. Plus: response shape KAT asserting `price` is a decimal string (NOT a JS number); IngestionLog volume KAT asserting ONE row per HTTP request (not per symbol); URL-sanitization KAT asserting `apikey` never appears in `IngestionLog.error_detail` even on failure. |
