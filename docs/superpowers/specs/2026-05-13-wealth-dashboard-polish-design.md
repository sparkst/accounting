...
```ts
// anchor = index of first date in the returned window for which close is non-null (per-series)
// Each benchmark/overlay series anchors independently
// ZERO-ANCHOR GUARD: if close[anchor] is null, zero, or Decimal('0'), return empty series
...
- **Hard 25s wall-clock cap:** After `Date.now() - startMs > 25_000`, abort any remaining symbols in `toRefresh` (mark them as skipped), finalize IngestionLog with current counts, and return the canonical 200 shape. This prevents Workers termination from leaving `in_progress` IngestionLog rows. `startMs` is captured at the top of the handler before any async work.
...
**`day_delta_pct` guard:** If `prior_close IS NULL` or `prior_close = '0.00000000'` → return `null`. If `price IS NULL` → return `null`. Frontend renders em-dash for null. Never divide by zero.
...
In the idle state: `role='status'` has empty textContent (no sr-only text, per §3.4.12 — no AT announcement on initial page load). `role='alert'` has empty textContent. When state transitions to 'error', content is inserted into the already-present `role='alert'` element — the assertive announcement fires. When error clears, the `role='alert'` content is removed (empty textContent again).
...
**Polling max-retry cap:** The client MUST NOT poll indefinitely. After `MAX_RETRIES = 6` polls (6 × `CLIENT_RETRY_MS` = 30s), transition to the error state with message "Could not refresh prices — refresh timed out, showing last known." Add `MAX_RETRIES = 6` to `src/lib/wealth-constants.ts`. State machine step 4 becomes: "If HTTP 202 and `pollCount < MAX_RETRIES` → increment `pollCount`, wait `CLIENT_RETRY_MS`, re-poll. If `pollCount >= MAX_RETRIES` → transition to error state."
...
3. If repriced-today returns HTTP 200 → `idle`; call `onRefreshed` callback
4. If repriced-today returns HTTP 202 → body is `{status: 'in_progress'}`; banner stays `refreshing`; client polls after `CLIENT_RETRY_MS` (5s from `src/lib/wealth-constants.ts`). **Detect by `response.status === 202`** (HTTP status code), not by reading a JSON field — the 202 body shape is distinct from all 200 shapes.
5. If repriced-today returns HTTP 200 with `error_code` non-null → `error` state with retry button
6. If repriced-today returns HTTP 200 with all `skipped` and `refreshed === 0` (no stale symbols) → `idle` immediately
...
export const PAYLOAD_THRESHOLD_BYTES = 2_000_000;   // Weekly downsample trigger (informational)
