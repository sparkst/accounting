...
### 2.3a Deployment constraint — Cloudflare Free 5-cron-per-account cap (operator decision 2026-05-19)
The Cloudflare account is on the Free plan, which caps cron triggers at **5 per account**. `wrangler.worker.toml` originally declared 7 (2 CRM + 5 wealth); the schedules API rejects >5 (`/workers/scripts/sparkry-crm-cron/schedules` → account-plan-limits). The cron-Worker *script* still deploys; only schedule registration is capped. Operator decision: stay on Free, keep 5 — the 2 CRM crons, `"7 10 * * *"` (REQ-WC-006 Plaid sync — the root-cause fix; non-negotiable), `"30 7 * * *"` (REQ-WC-013 price ingest), `"0 12 * * *"` (REQ-WC-018 wealth backup). **Dropped from the schedule:** `"0 14 * * MON"` (REQ-WC-007 stale-Item alert — superseded: Tier B re-attempts stale Items every login, A′ emails on drift) and `"0 */6 * * *"` (REQ-WC-005 abandoned-placeholder cleanup — low impact, manual if needed). The `worker.ts scheduled()` dispatch for both is retained, so re-adding them to the toml (e.g. on Workers Paid → 250/account) restores them with zero code change. REQ-PS-001 acceptance for the cron-Worker is satisfied at 5 schedules with `"7 10 * * *"` present.
