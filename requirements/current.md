...
| REQ-SEN-003 | Ingestion recency: every expected `ingestion_log` source must have a `status='success'` row within 26h — else `sev2`. Expected sources are **derived from active items** (`plaid_balance:<inst>` for all; `plaid_tx:<inst>` for register scope; `plaid_investments:<inst>` + `wealth_cloud:plaid_balance` for wealth scope) plus static daily sources (`stripe`, `shopify`), so a newly linked institution is monitored automatically and a retired wealth surface stops being expected (its producer only logs when wealth items exist). A recent `failed` row does not satisfy the assertion. When >1 active item shares an institution name (their log source keys collapse), a `sev3` `ingest_source_ambiguous` violation flags the degraded per-item coverage. |
