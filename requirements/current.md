...
- **Schema prerequisite:** add a `fetched_at` column (INTEGER epoch-ms, NOT NULL, default `unixepoch()*1000`) to `plaid_account_balance_snapshot`. `snapshot_date` remains a DATE (one row per account per UTC day; the REQ-WD-003 net-worth merge stays per-date); `fetched_at` records when that row's value was actually retrieved, enabling sub-day staleness and fresher-wins.
- Acceptance: on authenticated `/wealth` load, the client asynchronously POSTs `/wealth/api/brokerage/refresh-plaid-balances` (cookie-guarded; browser-facing — WEALTH_INTERNAL_KEY NOT applicable per REQ-WC-012).
  - **Eligibility:** an Item is processed iff it is *active* (`plaid_item.status='active'` AND `access_token_encrypted != 'REVOKED'`) AND its `last_sync_status` is NOT a terminal error (`ITEM_LOGIN_REQUIRED`, `INVALID_CREDENTIALS`, `INVALID_ACCESS_TOKEN`) AND its newest mapped-account snapshot's `fetched_at` is older than `PLAID_LOGIN_REFRESH_TTL_S` (constant, default `14400` = 4h; hardcoded constant, not a secret). Terminal-status Items are counted in `skipped_items`, never re-called (prevents per-login runaway Plaid billing on a broken Item). Items fresher than the TTL are skipped.
  ...
  - **Non-blocking:** the page renders immediately from last-known data; on resolve it bumps a reactive `refreshCount` `$state` that `<NetWorthChart>`/`<TopHoldingsTable>` consume via `$derived` to re-fetch (the same `onRefreshed` contract REQ-WD-008 uses — defined here since REQ-WD-008 only references it: parent increments `refreshCount`, children re-query on the derived key). A Miniflare integration test with a simulated 5s Plaid latency asserts the chart renders from last-known data within 2s of navigation, before the POST resolves.
  - **Concurrency:** WEALTH_KV lock key `plaid_refresh:<sha256(email)>` written with `expirationTtl: 120` (bounds worst-case multi-Item sync; auto-recovers from a crashed Worker — never a permanent lockout). A call that finds the lock held returns HTTP 202 `{status:'in_progress'}`; the client waits 5 s (`CLIENT_RETRY_MS = 5000`, the same interval the REQ-WD-008 banner uses) and re-issues once — if still 202, it renders last-known and stops (no infinite poll).
  ...
  - **Observability:** for each Item that actually calls Plaid, write one `ingestion_log` row (`source='plaid_login_refresh'`, `records_processed`=account rows written, `status`='success'|'error'); write one `audit_events` row per non-idempotent run (`entity_type='plaid_login_refresh_run'`, `entity_id`=per-run UUID).
...
- Non-Goals: real-time/streaming balances; refreshing Items fresher than the TTL or with terminal status; blocking page render on Plaid; changing the nightly cron (REQ-PS-001/REQ-WC-006). Window note (intentional, not a defect): with TTL=4h and cron at 10:07 UTC, an Item is login-refresh-eligible from ~14:07 UTC onward — covering afternoon/evening US sessions; the morning is already covered by the cron baseline.

## REQ-PS-003: Per-account baseline-drift alerting (cross-cutting, A′)
- Acceptance: after **every** Plaid balance write (nightly cron per REQ-WC-006 AND login refresh per REQ-PS-002), compare the just-written value against that account's **baseline** = the `plaid_account_balance_snapshot` row for the same `account_id` written strictly before the current one (regardless of which path wrote it; first-ever snapshot has no baseline → no alert).
  - **Sign normalization (explicit):** `normalize(b, plaid_account_type) = -b if plaid_account_type ∈ {credit, loan} else b`. Apply to BOTH sides: `delta = normalize(new) − normalize(baseline)`; `delta_pct = delta / abs(normalize(baseline)) × 100` computed with Decimal.js (ROUND_HALF_UP, scale 4). **Zero-baseline guard:** if `normalize(baseline) == 0` and `new != 0` → apply only the absolute threshold (skip pct, no div-by-zero); if both 0 → no alert.
