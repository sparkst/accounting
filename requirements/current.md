...
| REQ-FIX-PLD-007 | Wealth D1 push pages actionably on a wholly-unmapped Item (incident 2026-08-17: two same-name `Bank of America` Items, the superseded one paged daily for 9 days as `Bank of America: D1 skipped ALL 1 row(s)`). Contract unchanged — one wholly-unmapped Item among many still pushes every sibling, flags the batch, exits 1 → OnFailure. The page/`ingestion_log` detail MUST name the local Item id, the Plaid `item_id`, and every unmapped `plaid_account_id` with its name·mask·subtype label; when a healthy same-institution sibling Item resolved rows in the same run, the message MUST flag `SUPERSEDED` and carry `POST /api/plaid/disconnect/<local id>` for the failing Item only. Two same-institution Items that both fail to resolve get no hint. Test covers the mixed batch. |
...
| REQ-FIX-ING-021 | Stripe adapter ingests only settled money (cash basis): charges with status other than `succeeded` (failed, pending) and refunds with status `failed`/`canceled` are skipped (`records_skipped`), never inserted as income/expense rows. Stripe Billing retries each emit a distinct failed `ch_` object; before this rule 4 failed $8.00 Substack subscription retries (Jul 2026) landed as phantom income, inflating Sparkry B&O gross receipts by $32.00. Fixture mirrors the Jul-28 2026 payout: 7 succeeded charges gross $423.00 + 4 failed retries → register income exactly $423.00. |
...
| REQ-BNO-007 (code `REQ-BNO-CHK-007`) | Entity-vs-account commingling: every in-period receipt (`amount>0`) whose receiving account — keyed off the `payment_method` label, since the register has no account FK — is owned by a different `entity` than the row is FLAGGED, in either direction (business income into a personal account, e.g. Sparkry LLC Substack revenue paying out to personal Chase ...3894; or personal income into a business account). Account ownership is read from the `account` table (`payment_method` → `entity`), the authoritative self-maintaining source. The check is cross-entity: it scans every non-rejected receipt across all entities, not just the filing entity. A `payment_method` label with no matching `account` row is reported as "unmapped, cannot verify" and also FAILs — never silently passed (mirrors REQ-BNO-CHK-006). Rows with no `payment_method` (e.g. a Stripe charge; its payout row carries the label) are skipped. Detector for issue #76; canonical correct-destination from #61. |

## Digest email leg (2026-08-25) — REQ-DIG-EML-*

| REQ-ID | Requirement |
|--------|-------------|
| REQ-DIG-EML-001 | On every digest send (both the `📊 Wealth Snapshot` / `PERSONAL ACCOUNTS` pulse and the `🏢 Business Accounts` / `BUSINESS ACCOUNTS` pulse), in addition to the Telegram severity-webhook POST, send ONE email per digest to ALL configured recipients — a single Resend send carrying the full to-list. Rides the existing Resend mailer (`src/reports/report_email.py`); no n8n changes. |
| REQ-DIG-EML-002 | The email body is byte-identical to the Telegram message text (the exact string passed as `message` to `build_payload_dict`), sent as plain text plus the existing monospace `<pre>` HTML wrapper (`build_html_body`). Subject is exactly `<DIGEST NAME> - <YYYY-MM-DD>` with a plain hyphen; digest names are `PERSONAL ACCOUNTS` and `BUSINESS ACCOUNTS` (e.g. `PERSONAL ACCOUNTS - 2026-08-26`). |
